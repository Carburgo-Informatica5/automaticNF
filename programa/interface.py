...
class interfaceMonitoramentoNF(TKMT.ThemedTKinterFrame):
    ...
    def configurar_interface(self):
        ...
        # Status Geral
        self.frame_status = ttk.LabelFrame(frame_principal, text="Status do Sistema", padding="5")
        ...
        barra_rolagem = ttk.Scrollbar(frame_log, orient=tk.VERTICAL, command=self.texto_log.yview)
        barra_rolagem.grid(row=0, column=1, sticky=(tk.N, tk.S))
        self.texto_log["yscrollcommand"] = barra_rolagem.set

        # Estatísticas
        self.frame_estatisticas = ttk.LabelFrame(frame_principal, text="Estatísticas", padding="5")
        ...
        tk.Button(frame_controle, text="Pausar", command=self.pausar_processamento, **estilo_botao).grid(row=0, column=0, padx=5)
        tk.Button(frame_controle, text="Continuar", command=self.Retomar_processamento, **estilo_botao).grid(row=0, column=1, padx=5)
        tk.Button(frame_controle, text="Gerar Relatório", command=self.gerar_relatorio, **estilo_botao).grid(row=0, column=2, padx=5)
    ...
    def atualizar_interface(self):
        ...
        while True:
            try:
                ...
                self.texto_log.insert(tk.END, f"{evento['timestamp']} - {evento['descricao']}\n")
