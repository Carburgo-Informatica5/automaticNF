...
class bravos:
    ...
    def v_error_spotter(self, string):
        for stub in self.listof_stubs:
            if stub in unidecode(string).lower(): self.v_message_bundle.append(string)
    ...
    def __init__(self, config: dict, m_queue: DANImail.Queue) -> None:
        ...
        self.config = config
        try:
            self.v_config = yaml.safe_load(open(config["dani_vc"], 'rb').read().decode())
            self.v_queue = DANImail.Queue(self.v_config)
            self.v_queue.push(self.m_queue.make_message()
                        .add_text("Teste do coletor de erros.", tag="h1") 
                        .add_text("Este não é garantido funcional, erros podem estar omitidos do log.")
                        ,DANImail.WriteTo.header)
            self.listof_stubs: list[str] = list()
            for stub in self.v_config["stubs"]: self.listof_stubs.append(unidecode(str(stub)).lower())
            print(self.listof_stubs)
        except:
            self.m_queue.push(m_queue.make_message()
...
# Função para atualizar a janela do programa
    def busy_getWindowWithTitle(self, title: str, timeout_lenght: int) -> window.Win32Window:
...
# Caso erro na janela do programa ele executa o ALT+F4 (fecha o programa)
    def kill_warning(self, timeout_lenght: int):
        ...
        while time.time() - start_time < timeout_lenght:
            ...
            for title in listof_tittles: listof_windows += window.getWindowsWithTitle(title)
            for warn in listof_windows:
                ...
                if 'error' in warn.title.lower(): 
                    gui.sleep(1)
                    gui.press('enter')
                    if(len(window.getWindowsWithTitle("error")) > 0): gui.press('enter')
                else: gui.hotkey('alt', 'f4')
                start_time = time.time()
...
# Fecha o Popup do programa
    def close_document_popup(self, timeout_lenght: int):
        ...
        while True:
            ...
            print(f"\r > Segundos desde a ultima janela: {int(time.time() - last_document)}       ", end='')
            try:
            ...
            try:
                ...
                if time.time() - last_document > timeout_lenght: break
                continue
    ...
    def __find_login(self, timeout_lenght) -> window.Win32Window:
        ...
        while True:
            ...
            try:
                ...
                self.m_queue.push(self.m_queue.make_message()
                    .add_img(gui.screenshot())
                    .add_text(f"Encontrado Janela de update, dormindo por {timeout_lenght} segundos.", tag="h1 style=\"color: white;\"", tag_end="h1"))
                timeout_lenght = TIMEOUT_LENGHT_UPDATE
                gui.sleep(TIMEOUT_LENGHT_UPDATE/16)
            except: ()

            try: return window.getWindowsWithTitle("Identificação")[0]
            except: ()

# Pega as informações do user e faz o login dele 
    def acquire_bravos(self, exec:str="C:\\BravosClient\\FabricaVWClient.exe"):
        subprocess.run(["powershell.exe", "-Command", "Get-Process | Where-Object {$_.Path -like \"*bravos*\"} | Stop-Process"], shell=True)
    ...
    def upload(self, dealers: list[dict]) -> bool:
        ...
        for dealer in dealers:
            ...
            dealer_msg = (
                ...
                .set_color("green")
                .add_text(f"{dealer['unit']}:", tag="h1 style=\"color: white;\"", tag_end="h1")
                .insert_raw("<br>")
            ...
            try:
                ...
                if "error 103." in clipboard:
                    dealer_msg.set_color("red")
                    dealer_msg.add_text("Erro no bravos!", tag="h2")
                    dealer_msg.add_text(f"Pulando {dealer['unit']} por conta de um erro 103...")
                    dealer_msg.add_img(gui.screenshot())
                    dealer_msg.add_text("Reinciando Bravos...")
                    bravos = self.acquire_bravos()
                    self.m_queue.push(dealer_msg)
                ...
                logs = copyof_logs
                (   dealer_msg
                    .set_color("red" if "ERRO" in logs else "green")
                    .insert_raw("<br>")
                    .add_text(f"    > Resultados da verificação de {dealer["unit"]} retorna: ", tag="h2")
                    .insert_raw("<br>")
                ...
                print(f" > Hibernado por {self.WAITING_PERIOD_LENGTH} segundos de modo a esperar o download concluir.")
                for sec in range(self.WAITING_PERIOD_LENGTH):
                    while True:
                        try:
                            popup:window.Win32Window = window.getWindowsWithTitle("Confirm")[0]
                            popup.activate()
                            gui.press("enter")
                            gui.sleep(1)
                        except: break
                ...
                (   dealer_msg
                    .set_color("red" if ("ERRO" in logs or "não encontrado" in logs) else "green")
                    .insert_raw("<br>")
                    .add_text(f"    > Logs de execução de {dealer["unit"]} retorna:", tag="h2")
                    .insert_raw("<br>")
                    .add_text(logs)
                )
                
                for log in logs: self.v_error_spotter(log)
                if len(self.v_message_bundle) > 1:
                    self.v_queue.push(self.v_queue.make_message()
        ...
        try:
            ...
            self.m_queue.push(self.m_queue.make_message()
                    .add_text(["Erro no v_eh!" "Mensagem não enviada.", self.m_queue.print_trace()]))
        return True
